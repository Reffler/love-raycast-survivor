...
// LÖVE pixel-shader adapter. Input is the native low-resolution scene canvas.
...
//--------------------------------------------------------------
// #define CRTS_WARP 1 // Disabled: straight screen edges, no corner vignette.
//--------------------------------------------------------------
...
vec3 CrtsFetch(vec2 uv){
 ...
 // InputSize matches the canvas texture; no padded RetroArch texture area.
 // Move towards intersting parts
// uv+=vec2(0.5,0.5);
 // Non-shadertoy case would not have the color conversion
 return FromSrgb(Texel(Texture,uv).rgb);}
...
vec4 effect(vec4 color, Image image, vec2 uv, vec2 screenPosition)
{
	vec2 warp_factor;
	warp_factor.x = CURVATURE;
	warp_factor.y = (OutputSize.y / OutputSize.x) * warp_factor.x;
	warp_factor.x *= (1.0 - TRINITRON_CURVE);
	vec3 filtered = CrtsFilter(uv * OutputSize,
	...
	return vec4(ToSrgb(filtered), 1.0) * color;
} 
#endif
