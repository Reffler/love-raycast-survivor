#!/usr/bin/env python3
"""Capture generated waterfall and time native water views with/without slopes."""
import argparse, os, subprocess, tempfile
from pathlib import Path
root = Path(__file__).resolve().parents[1]
app = Path(tempfile.mkdtemp(prefix="flow-render-"))
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument('--output', type=Path, default=root / "artifacts/part2-7-flow")
parser.add_argument('--seed', type=int, default=1337)
parser.add_argument('--support-only', action='store_true')
args = parser.parse_args()
output = args.output.resolve()
output.mkdir(parents=True, exist_ok=True)
for asset in root.iterdir():
    if asset.name != "main.lua" and asset.suffix in {".lua", ".glsl", ".ttf"}:
        (app / asset.name).symlink_to(asset)
(app / "main.lua").write_text((root / "main.lua").read_text() + "\n" + (root / "tools/check_flow_render.lua").read_text())
result = subprocess.run(["love", str(app)], capture_output=True, text=True, timeout=120,
    env={**os.environ, "SDL_VIDEODRIVER": "offscreen", "ALSOFT_DRIVERS": "null", "RAYCAST_OUTPUT": str(output),
         "FLOW_SEED": str(args.seed), "FLOW_SUPPORT_ONLY": "1" if args.support_only else "0"})
print(result.stdout, end="")
print(result.stderr, end="")
(output / "timings.txt").write_text(result.stdout)
raise SystemExit(result.returncode)
