...
function H:tangent(node)
  if node.tx then return node.tx,node.ty end
  self:resolve(node.x,node.y)
  local parent,drop=nil,0
  for dy=-1,1 do for dx=-1,1 do if dx~=0 or dy~=0 then
    local other=self:node(node.x+dx,node.y+dy)
    if other.height>node.height then
      self:resolve(other.x,other.y)
      local slope=(other.height-node.height)/math.sqrt((other.px-node.px)^2+(other.py-node.py)^2)
      if other.next==node and slope>drop then parent,drop=other,slope end
    end
  end end end
  local nextNode=node.next or node.spill
  local x,y=0,0
  if parent then
    local length=math.sqrt((node.px-parent.px)^2+(node.py-parent.py)^2)
    x,y=(node.px-parent.px)/length,(node.py-parent.py)/length
  end
  if nextNode then
    local length=math.sqrt((nextNode.px-node.px)^2+(nextNode.py-node.py)^2)
    x,y=x+(nextNode.px-node.px)/length,y+(nextNode.py-node.py)/length
  end
  local length=math.sqrt(x*x+y*y)
  node.tx,node.ty=length>1e-9 and x/length or 1,length>1e-9 and y/length or 0
