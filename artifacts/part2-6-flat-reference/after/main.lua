...
local CONFIG = {
  ...
  VIEW_DIST        = 256.0,             -- Max raycast distance (visibility cutoff)
...
-- ─────────────────── state variables ────────────────────────────────
local world = World.new(CONFIG.SEED, CONFIG.VIEW_DIST, {SEA_LEVEL=CONFIG.SEA_LEVEL,
...
local function getHighestFloorUnder(x, y, minHeight, maxHeight)
  ...
  for ix = minX, maxX do
    for iy = minY, maxY do
      local wh = world:height(ix - 1, iy - 1)
      if wh then
  ...
  for ix = minX, maxX do
    for iy = minY, maxY do
      ...
      if wh then
        ...
        if tooHighToStep and footH < (wh - CONFIG.GROUND_EPS) then
          return true
        end
      end
    end
...
local function updatePhysics(dt)
  ...
  if flying then
    ...
    currentFloor = getHighestFloorUnder(px, py)
    eyeHeight = math.max(currentFloor + CONFIG.CAM_HEIGHT, eyeHeight + vertical * targetSpeed * dt)
    yVel = 0
    return
  end
