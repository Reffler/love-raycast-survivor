...
p.add_argument('--output',type=Path,default=Path('artifacts/geometry-aa'))
args=p.parse_args()
...
(app/'before.glsl').symlink_to(root/'artifacts/geometry-aa/before.glsl')
(app/'main.lua').write_text('function love.errorhandler(message) print(message); return function() return 1 end end\n'+(root/'main.lua').read_text()+'\n'+(root/'tools'/('aa_'+args.mode+'.lua')).read_text())
