...
local CONFIG = {
  ...
  AA_ENABLED       = true,              -- F5: toggle four coverage rays
...
function love.draw()
  ...
  if raycastShader:hasUniform('aaEnabled') then raycastShader:send('aaEnabled',CONFIG.AA_ENABLED) end
  ...
  if CONFIG.DEBUG_ENABLED and hudElapsed >= 0.1 then
    ...
    hud:clear()
    hud:add(("Pitch: %.1f° | FOV: %d° | AA [F5]: %s"):format(math.deg(pitch), CONFIG.FOV_DEG, CONFIG.AA_ENABLED and "4x" or "off"), 0, 0)
