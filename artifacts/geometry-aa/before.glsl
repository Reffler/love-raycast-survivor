...
vec4 effect(vec4 color, Image dummy, vec2 tc, vec2 sc) {
  ...
  if (camPos.z>maxHeight && ray.z>=0.0) return environmentColor(skyColor(ray));
  ...
  for (int i=0;i<MAX_DDA_STEPS;++i) {
    ...
    if (waterfall && !wall) return environmentColor(atmosphericColor(WATER*lightTint,ray,entry));
    if (waterHit>=entry && waterHit<=end && !wall && (top<entry || waterHit<top)) {
      return environmentColor(atmosphericColor(WATER*lightTint,ray,waterHit));
    }
  ...
  if (solidHit>=0.0) {
    ...
    if (solidTexture>=0.0) {
      ...
      // Analytic footprint avoids derivatives of fractured UVs or divergent DDA hits.
      // Grazing faces cover more texture per pixel. LOD 0 retains nearest magnification.
      float pixelAngle=2.0*max(tanHalfHFOV/love_ScreenSize.x,tanHalfVFOV/love_ScreenSize.y);
      float footprint=16.0*solidHit*pixelAngle/max(abs(dot(ray,solidNormal)),0.001);
      float lod=max(0.0,log2(max(footprint,1.0)));
      solidColor*=solidTexture<0.5 ? textureLod(dirtTex,uv,lod).rgb
        : solidTexture<1.5 ? textureLod(grassSideTex,uv,lod).rgb : textureLod(grassTopTex,uv,lod).rgb;
    }
    // Sun and moon share one bounded shadow ray; only the above-horizon light contributes.
    ...
    float ndotl=max(dot(solidNormal,lightDirection),0.0),visibility=1.0;
    if (ndotl>0.0 && directStrength>0.0 && lightDirection.z>0.20791169 && shadowsEnabled && shadowDistance>0.0)
      visibility=sunVisibility(shadowReceiver(hit,solidNormal),lightDirection);
#ifdef SHADOW_STATS
    ...
    vec3 shaded=solidColor*lighting*ambientOcclusion(hit,solidNormal);
    return vec4(atmosphericColor(shaded,ray,solidHit),1.0);
#endif
  }
  return environmentColor(skyColor(ray));
}
