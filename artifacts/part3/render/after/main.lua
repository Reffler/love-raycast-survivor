...
local function isPointColliding(x, y, footH, canStep)
  ...
  for ix = minX, maxX do
    for iy = minY, maxY do
      if world:overlaps(ix-1,iy-1,foot+CONFIG.GROUND_EPS,foot+CONFIG.CAM_HEIGHT) then return true end
    end
  end
  return false
end
...
local function getLowestCeiling(x,y,head)
  ...
  for ix=math.floor(x-r),math.floor(x+r) do for iy=math.floor(y-r),math.floor(y+r) do
    ceiling=math.min(ceiling,world:ceilingAbove(ix,iy,head-CONFIG.GROUND_EPS) or math.huge)
  end end
  return ceiling
end
