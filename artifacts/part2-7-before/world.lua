...
function World.new(seed,viewDistance,terrainConfig)
  ...
  for i=1,self.slots^2 do
    self.chunks[i]={data=ffi.new('uint16_t[256]'),surface=ffi.new('uint16_t[256]'),x=math.huge,y=math.huge,index=i,previous=0,next=0,queued=false,requestX=0,requestY=0,maximum=0}
  end
...
function World:generateChunk(cx,cy)
  local chunk=self.chunks[self:slot(cx,cy)]
  chunk.maximum=self.terrain:generate(cx,cy,chunk.data,chunk.surface)
