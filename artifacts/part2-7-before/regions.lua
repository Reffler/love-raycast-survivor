      ...
      for x=-3,R.CORE+3 do
        ...
        if amount>=8 and drain[i]>self.config.SEA_LEVEL then
          ...
          if j>=0 then
            ...
            local lower=H.level(downstream)
            if lower<level then
              -- Locate lip on unchanged bent path using uncarved terrain support.
              local lip,bestDrop,steepest=nil,-math.huge,0.5
              local previous=G.height(self.config,ox+ax,oy+ay)
              for step=1,32 do
                local t=step/32;local xx,yy=Fluid.path(ax,ay,mx,my,bx,by,t)
                local ground=G.height(self.config,ox+xx,oy+yy)
                if previous>=level and ground<level then lip=(step-1+(previous-level)/(previous-ground))/32 end
                if previous-ground>bestDrop then bestDrop,steepest=previous-ground,t end
                previous=ground
              end
              lip=lip or steepest
              region.drops[#region.drops+1]={ax=ax,ay=ay,mx=mx,my=my,bx=bx,by=by,upper=level,lower=lower,amount=amount,lip=lip}
              carve(ax,ay,mx,my,lower,amount,false)
              carve(mx,my,bx,by,lower,amount,false)
            else
              carve(ax,ay,mx,my,level,amount,false)
              carve(mx,my,bx,by,lower,amount,false)
            end
          else carve(ax,ay,ax,ay,node.base,amount,true) end
          coroutine.yield()
        end
      end
      coroutine.yield()
