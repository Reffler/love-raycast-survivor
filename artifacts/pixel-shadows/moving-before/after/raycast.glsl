...
vec3 skyColor(vec3 ray) {
  vec3 sky=skyTint*mix(1.0,0.72,clamp(ray.z,0.0,1.0));
  // Square sun and opposite moon, fixed to the world rather than the camera.
  vec2 celestial=vec2(ray.y,dot(ray,vec3(sunDirection.z,0.0,-sunDirection.x)));
...
vec4 effect(vec4 color, Image dummy, vec2 tc, vec2 sc) {
  ...
  if (camPos.z>maxHeight && ray.z>=0.0) return vec4(skyColor(ray),1.0);
  ...
  for (int i=0;i<MAX_DDA_STEPS;++i) {
    ...
    if (waterfall && !wall) return vec4(WATER*lightTint,1.0);
    if (waterHit>=entry && waterHit<=end && !wall && (top<entry || waterHit<top)) {
      return vec4(WATER*lightTint,1.0);
    }
  ...
  if (solidHit>=0.0) {
    ...
    return vec4(solidColor*lightTint*ambientOcclusion(hit,solidNormal),1.0);
  }
  return vec4(skyColor(ray),1.0);
}
