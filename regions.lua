...
function R.new(config)
  local self=setmetatable({config=config,cache={},clock=0,builds=0,cpuSeconds=0,maxBuildSeconds=0,caveSeconds=0,maxCaveSeconds=0,
...
function R:build(region,rx,ry)
  ...
  Caves.build(region,self.config,rx,ry,function(x,y)
    local fx,fy=(x-ox)/32+R.HALO,(y-oy)/32+R.HALO
    local ix,iy=math.floor(fx),math.floor(fy);local u,v=fx-ix,fy-iy;local k=iy*N+ix
    assert(ix>=0 and iy>=0 and ix<N-1 and iy<N-1,'Cave graph exceeded height halo')
    return lerp(lerp(heights[k],heights[k+1],u),lerp(heights[k+N],heights[k+N+1],u),v)
  end)
  self.lastHydrologyNodes=hydrology.count
  self.caveSeconds=self.caveSeconds+region.caveBuildSeconds
  self.maxCaveSeconds=math.max(self.maxCaveSeconds,region.caveBuildSeconds)
